...
def solve_heat1d(a: np.ndarray, b: np.ndarray, c: np.ndarray, grid: np.ndarray, R: float) -> np.ndarray:
    ...
    for i in prange(grid.shape[0]):
        ...
        solution[0] = set_initial_temperature_distribution(0)
        solution[-1] = set_initial_temperature_distribution(0)
        solutions[i] = solution
    return solutions


@njit(fastmath=True, parallel=True)
def set_initial_temperature_distribution(ax, L=1):
    return 1 - ax ** 2 / L ** 2


@njit(fastmath=True, parallel=True)
def set_temperature_distribution(x: np.ndarray, y: np.ndarray, u: np.ndarray):
...
def solve_heat2d(x: np.ndarray, y: np.ndarray, time: np.ndarray, sigma: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    c: np.ndarray = a.copy()
    a[0] = set_initial_temperature_distribution(0)
    c[-1] = set_initial_temperature_distribution(0)
